...
| 版本 | 功能 | 技术要点 |
|------|------|----------|
| M6 | Web 配置中心基础 | Flask + Jinja2 + TinyMCE |
| V2 | 操作类型章节绑定 | action_library 嵌套结构、批量删除 |
| V3 | 核心字段同步 | 关键词匹配、别名自动更新 |
| V4 | WangEditor 替换 | MIT 协议编辑器、自动同步 |
| V5 | 全量覆盖模式 | batch_save 重构、配置清理 |
| V6 | 操作类型自动识别 | 预览识别、冲突处理 |

### 渲染策略演进

- **双轨渲染**：支持 docxtpl 模板 + python-docx 内置渲染
- **AUTO 策略**：模板优先，失败自动回退
- **输出对齐**：实施总表 6 列、2.1.x 编号、日期转换

### 历史缺陷修复

- **缺陷 #1**：实施总表列异常（46315、Unnamed 过滤）
- **缺陷 #2**：第2部分层级与命名不符（2.1 实施总表 → 2.1 详细实施步骤）

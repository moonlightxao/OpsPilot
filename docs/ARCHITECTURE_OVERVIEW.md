...
│   ├── images/                  # Web 上传图片存储（M6 新增）
│   └── backups/                 # 配置备份目录（M6 新增）
├── templates/
...
│   ├── test_web_api_v2.py       # Web API V2 测试
│   ├── test_batch_save_v5.py    # V5 批量保存测试
│   ├── test_action_type_recognition_v6.py  # V6 操作类型识别测试
│   └── ...                      # 其他测试
├── output/                      # 输出目录
│   ├── report.json              # 中间态数据
│   └── 实施文档.docx            # 生成文档
...
### 3.5 Web 配置中心 - `src/web/`

**职责**：提供可视化配置管理界面，替代 YAML 文件直接编辑。

**技术栈**：Flask + Jinja2 + WangEditor（富文本编辑）
...
| API 端点 | 方法 | 功能 |
|----------|------|------|
| `/api/config` | GET/PUT | 完整配置读写 |
| `/api/config/priority-rules` | GET/PUT | 章节优先级配置 |
| `/api/config/priority-rules/chapter` | POST | 添加新章节 |
| `/api/config/priority-rules/chapter/<name>` | DELETE | 删除章节 |
| `/api/config/priority-rules/batch-delete` | POST | 批量删除章节 |
| `/api/config/action-library` | GET/PUT | 操作类型配置 |
| `/api/config/action-library/chapter/<chapter>` | GET/PUT | 章节操作类型配置 |
| `/api/config/action-library/chapter/<chapter>/<action>` | PUT/DELETE | 单个操作类型 |
| `/api/config/action-library/batch-delete` | POST | 批量删除操作类型 |
| `/api/config/sheet-column-mapping` | GET/PUT | 列映射配置 |
| `/api/config/sheet-column-mapping/<sheet>` | GET/PUT/DELETE | 单个 Sheet 列映射 |
| `/api/config/sheet-column-mapping/batch-delete` | POST | 批量删除列映射 |
| `/api/config/batch-save` | POST | V5: Excel 一键保存（全量覆盖） |
| `/api/config/batch-save-action-types` | POST | V6: 操作类型批量保存 |
| `/api/config/sync-core-fields` | POST | 核心字段同步 |
| `/api/upload/image` | POST | 图片上传（富文本） |
| `/api/upload/excel` | POST | Excel 列名识别 |
| `/api/upload/excel/preview` | POST | V6: Excel 预览（含操作类型识别） |
| `/api/backups` | GET | 备份列表（最近 10 条） |
| `/api/backups/create` | POST | 手动创建备份 |
| `/api/backups/<filename>` | GET/DELETE | 备份内容/删除 |
| `/api/backups/<filename>/restore` | POST | 回滚到指定版本 |
...
class ConfigService:
    ...
    def get_sheet_column_mapping(self) -> dict: ...
    def batch_save_sheets(self, sheets: list) -> dict: ...  # V5 全量覆盖
    def batch_save_action_types(self, action_types: dict) -> dict: ...  # V6 新增
...
### 3.6 V5 功能：Excel 一键保存全量覆盖

**版本**：V5（2026-03-01）

**需求背景**：用户反馈 Excel 一键保存后配置累积/膨胀，原有增量更新模式无法清理历史配置。

**功能说明**：
- 上传 Excel 后，全量覆盖 `sheet_column_mapping` 和 `priority_rules`
- 全量重新生成 `core_fields`，清理不再使用的字段
- 保留 `action_library` 不变（操作类型需单独管理）
- API 返回 `deleted` 字段记录被清理的配置项
...
**需求背景**：用户希望导入 Excel 后自动识别每个 Sheet 的操作类型列并保存到配置。

**功能说明**：
- 预览时自动识别每个 Sheet 的「操作类型」列（支持别名匹配）
- 返回识别到的操作类型列表，按 Sheet 分组
- 已存在的操作类型显示冲突标记，用户可选择覆盖或跳过
- 支持批量保存操作类型到 `action_library`
...
| `test_web_api.py` | Web 配置中心 API 测试 |
| `test_web_api_v2.py` | Web API V2 测试（章节绑定、批量删除、核心字段同步） |
| `test_batch_save_v5.py` | V5 批量保存全量覆盖测试 |
| `test_action_type_recognition_v6.py` | V6 操作类型自动识别测试 |
